...
The first model is imitation learning. You drive clean routes, the recorder saves telemetry and your controller inputs, then the trainer learns to predict those inputs from the telemetry frame. Frames are enriched with short-term motion context before training and driving, because a car's next good action depends on recent speed change, lateral motion, grip load, and road direction rather than a single static sample.
...
Human override has highest priority in the drive loop. Keyboard polling can turn `W/A/S/D`, arrows, `Space`, `Q`, `E`, and Shift into live control targets. Telemetry input that differs from the program's last output is treated as physical user control, so the virtual controller is neutralized and the online learner trains on the user's action instead of the model's action. Human override frames are expert samples with a larger learning weight than normal self-training; the neutral release-hold frames are not treated as expert examples. A short neutral hold follows user release so the model does not immediately counter-steer against the user.
...
The reward score changes how strongly the online learner trains on the last action. Negative transitions also adjust the target by reducing throttle, trimming steering, or adding a little braking before the sample is learned. Expert override samples bypass reward target adjustment and train directly on the user's controls. Exploration has two layers: curiosity-driven action bursts for discovering broader behavior, and a low-amplitude entropy layer that continuously samples nearby steering/throttle choices. Saved online models restore a configurable exploration floor at startup so they keep testing unless `--no-explore` is used.

Session logs include interval summaries and a final scorecard with average reward, speed, lane error, terrain distribution, override frames, stuck events, and crash events. This is the main tuning feedback loop for comparing reward-profile changes.
